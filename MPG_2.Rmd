...
packages <- c(
  "flexdashboard", "data.table", "clifro", "openair", "ggplot2",
  "stringr", "DT", "gridExtra", "lubridate", "plotly",
  "tidyr", "shiny", "units", "ggpubr", "dplyr"
)
...
if (os_type == "Windows") {
  message("You're on a PC")
} else if (os_type == "Darwin") {
  message("You're on a Mac")
} else {
  message(paste("You're on", os_type))
}

if (os_type == "Darwin") {
  basedir <- paste0("~/Desktop/FluxTowerCheck/", site, "/")
} else {
  basedir_p1 <- file.path(Sys.getenv("USERPROFILE"), "Desktop")
  basedir <- paste0(basedir_p1, "/FluxTowerCheck/", site, "/")
}
...
# import data directly from google drive
soil_colnames <- fread(
  paste0(basedir, "/toa5/soil.csv"),
  header = TRUE,    
  skip = 1,          
  sep = ",",
  na.strings = c("-9999", "#NAME?"),
  select = 1:97,      # Only read columns 1–97
  nrows = 1           #  grab one row
)
...
soil <- fread(
  ...
  skip = 3,
  sep = ",",
  fill = Inf,  # <--- allow any number of columns per row
  na.strings = c("-9999", "#NAME?"),
...
#convert timestamp to posix
soil$TIMESTAMP <- as.POSIXct(soil$TIMESTAMP, format="%Y-%m-%d %H:%M:%S", tz="MST")
...
soil$date_time = lubridate::ymd_hms(soil$TIMESTAMP)
soil$date <- as.Date.POSIXct(soil$date_time, format="%Y-%m-%d")
soil <- subset(soil, !is.na(soil$date))
soil <- subset(soil, year(date) == yeartocheck)

# grab soil data columns 
## currently set up for Vcp
swc_columns <- c(grep("^VWC_", names(soil), value = TRUE), "TIMESTAMP")
soilT_columns <- c(grep("^T_", names(soil), value=TRUE), "TIMESTAMP")
soilC_columns <- c(grep("^SCO2", names(soil), value=TRUE), "TIMESTAMP")
...
soil1 <- rbind(soilSWC, soilT, soilCO2)
...
### grab sap columns (some sites require a second download and table)
sap_columns <- c(grep("^sap_", names(soil), value = TRUE), "TIMESTAMP")

# sub to sap columns only
sap_trees <- soil[, ..sap_columns]
colnames(sap_trees) <- str_replace(colnames(sap_trees), "(sap_J\\d+)([A-Za-z])", "\\1_\\2")

# convert to long format
sapTrees <- sap_trees %>%
  pivot_longer(-c(TIMESTAMP),names_to="IDcol") %>%
  separate(IDcol, c("metric","tree","probe"), sep="_")

sapTrees$date_time = ymd_hms(sapTrees$TIMESTAMP)
sapTrees$date <- as.Date.POSIXct(sapTrees$date_time, format="%Y-%m-%d")
...
dateRangeInput("daterange", "Date range:",
                 start = max(flux$date, na.rm = T) - weeks(2),
                 end   = max(flux$date, na.rm=T),
                 min = max(flux$date, na.rm=T) - weeks(2),
                 max = max(flux$date, na.rm=T))
...
radioButtons("power_dx", "Diagnostic variables:",
             c("panelT_1000x_Avg" = "panelT_1000x_Avg",
               "batt_1000x_Avg" = "batt_1000x_Avg",
               "battbank_Avg" = "battbank_Avg",   
               "panelV_Avg" = "panelV_Avg",
               "low12V_1000x_Tot" = "low12V_1000x_Tot",
...
# this creates and renders a table with date ranges for missing or out of range data. 
...
flux1$date <- as.Date.POSIXct(flux1$date_time, format="%Y-%m-%d")
  ...
  # key for plotting multiple biomet in a single category
   ...
   # use simple variable key to combine multiple related variables into a single radio button 
  ...
  c_diag <- flux1[date >= input$daterange[[1]] & date <= input$daterange[[2]], 
  ...
  power_diag <- flux1[date >= input$daterange[[1]] & date <= input$daterange[[2]], 
...
renderPlotly({
  ...
  rain <- subset(c_biomet_sub, variable == "rain_Tot")
  
  rain <- ggplot(rain, aes(date_time, value)) +
    geom_col() +
    labs(y = "total P. (mm)") +
    theme_bw()
  ...
  p3_5 <- ggplotly(rain) %>% layout(title = list(text = "Precip."))
...
# Wind direction from Sonic Anemometer
 renderPlot({
  recentflux <-  flux %>%
   ...
   wind<- with(recentflux, windrose(wnd_spd, wnd_dir_csat3,
...
dateRangeInput("daterange3", "Date range:",
                 start = max(soil1$date, na.rm = T) - weeks(2),
                 end   = max(soil1$date, na.rm = T),
                 min = max(soil1$date, na.rm = T)- weeks(2),
                 max = max(soil1$date, na.rm = T))
...
renderPlotly({
  soil1$depth <- factor(soil1$depth, levels = c("5", "10", "30", "60"))
  # Soil Faceted Plot
  soil_faceted_data <- soil1 %>%
  ...
  # Soil Selected Depth
  soil_selected_depth_data <- soil1 %>%
    filter((date >= input$daterange3[[1]] & date <= input$daterange3[[2]]) &
           metric == input$variable1_3 &
...
dateRangeInput("daterange4", "Date range:",
                 start = max(sapTrees$date, na.rm = T) - weeks(2),
                 end   = max(sapTrees$date, na.rm = T),
                 min = max(sapTrees$date, na.rm = T) - weeks(2),
                 max = max(sapTrees$date, na.rm = T))


radioButtons("variabletreenum", "Tree:",
             c("juniper 1" = "J1",
               "juniper 2" = "J2",
               "juniper 3" = "J3",
               "juniper 4" = "J4",
...
renderPlotly({
   ...
   sap_facetedPlot <- ggplot(sap_faceted_data, aes(date_time, value, colour = tree)) +
    ...
    #one sap trees in  plot
    #   sap_data <- sapTrees %>%
    #    filter((date >= input$daterange4[[1]] & date <= input$daterange4[[2]]) & 
    #  tree==input$variabletreenum)
    #   
  ...
  # Combine subplots
  plotly::subplot(p1, p3, p4, nrows = 3, shareX = TRUE, titleX = TRUE, titleY = FALSE) %>%
    layout(
      height = 1200,
      margin = list(
        l = 80,   # Left margin
