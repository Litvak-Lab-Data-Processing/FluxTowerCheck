...
for (i in seq_along(file_ids)) {
  ...
  file_url <- paste0("https://drive.google.com/uc?export=download&id=", file_ids[i])
  ...
  # Download with proper timeout and binary mode
...
radioButtons("power_dx", "Diagnostic variables:",
             c("panelT_1000_Avg" = "panelT_1000_Avg",
               "batt_1000_Avg" = "batt_1000_Avg",
               "battbank_Avg" = "battbank_Avg",   
               "panelV_Avg" = "panelV_Avg",
               "low12V_1000_Tot" = "low12V_1000_Tot",
               "watchdog_1000_Tot"= "watchdog_1000_Tot"))
...
# Wind direction from Sonic Anemometer
 renderPlot({
  recentflux <-  flux %>%
   ...
   wind4p5 <- with(recentflux, windrose(WS_ms_4p5, W_dir_4p5,
...
print(max(soil$date_time, na.rm = T))
```
...
dateRangeInput("daterange3", "Date range:",
                 start = max(soil$date, na.rm = T) - weeks(2),
                 end   = max(soil$date, na.rm = T),
                 min = max(soil$date, na.rm = T)- weeks(2),
                 max = max(soil$date, na.rm = T))
