...
file_ids <- c("152EEhXXcmjXgPLkLuNSrBexKayV0q0cR")  # file ID from google drive 
...
# get limit data 
lims <- read.csv(paste0(basedir, "SetVarLims.csv")) ## HERE 
...
soilT <- soil_t %>%
  pivot_longer(-c(TIMESTAMP),names_to="IDcol") %>%
  separate(IDcol, c("metric","pit","depth"), sep="_")
soilT$pit <- str_replace(soilT$pit, "BLU", "BLUG1")
soilT$pit <- str_replace(soilT$pit, "BLA", "BLAG1")

soilCO2 <- soil_C %>%
  pivot_longer(-c(TIMESTAMP),names_to="IDcol") %>%
  separate(IDcol, c(NA, "metric","pit","depth"), sep="_")
soilCO2$pit <- str_replace(soilCO2$pit, "Blu", "BLUG1")
soilCO2$pit <- str_replace(soilCO2$pit, "Bla", "BLAG1")
soilCO2$pit <- str_replace(soilCO2$pit, "O", "O1")
  ...
  for (var_name in names(ir_vars)) {
    ...
    if (!is.null(tab) && nrow(tab) > 0) {
      ...
      formatted_tab <- tab %>%
        ...
        select(Variable, start_time, end_time, Message)
...
flux1$date <- as.Date.POSIXct(flux1$date_time, format="%Y-%m-%d")
  ...
  # key for plotting multiple biomet in a single category
   simpVar_key <- data.frame("OldVariable"=c("AirTC_Avg", "CNR1TC_Avg", 
                                             ...
                                             "Rad_long_Up_Avg", "Rad_long_Dn_Avg", "NetRl_Avg", 
                                             "CG3UpCo_Avg","CG3DnCo_Avg",
                                             "wnd_dir_compass", "wnd_dir_csat3", "wnd_spd",
                                             "rslt_wnd_spd", "std_wnd_dir", "WS_ms_sentry_Avg",
...
radioButtons("sonic_dx", "Diagnostic variables:",
             c("CSAT_n_Tot" = "CSAT_n_Tot",
               "csat_warnings" = "csat_warnings",
...
radioButtons("variable2_3a", "Soil Pit:",
             c("B. gracilis 1"="BLUG1",
               "B. eriopoda 1" = "BLAG1",
               "Open P1" = "O1"))

radioButtons("variable2_3", "Soil Depth:",
             c("2p5cm (SWC & SoilT)"="2p5",
               "12p5cm (SWC & SoilT)"= "12p5",
               "22p5cm (SWC & SoilT)"="22p5",
               "37p5cm (SWC & SoilT)"="37p5",
               "52p5cm (SWC & SoilT)" = "52p5",
               "5 (Soil CO2)" = "5",
               "10 (Soil CO2)" ="10",
               "30 (Soil CO2)" = "30",
               "Avg (Soil CO2)" = "Avg"))
