...
## by default, show last 2 weeks, but have option to show from beginning of time
### Beginning of time may take a long time to render. 
yeartocheck <- 2025
...
#flux$Fc_wpl_IR <- ifelse(flux, flux$Fc_wpl > -50 | flux$Fc_wpl < 50, NA, flux$Fc_wpl)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# print start/stop dat times for periods > 4 hours with missing or out of range data.
# output$longmissingTable <- renderTable({
#   req(input$variable1)
# 
#   tab <- NULL
#   if (grepl("_IR", input$variable1)) {
#     if (grepl("Fc_wpl", input$variable1)) tab <- Fc_long_na_periods
#     if (grepl("LE_wpl", input$variable1)) tab <- LE_long_na_periods
#     if (grepl("Hc", input$variable1)) tab <- Hc_long_na_periods
#   }
# 
#   if (!is.null(tab) && nrow(tab) > 0) {
#     tab %>%
...
#     data.frame(Message = "No periods out-of-range for > 4 consecutive hours.")
#   }
# })
# 
# output$longmissingTab <- renderUI({
#   req(input$variable1)
#   tagList(
#     h4("Periods with more than 4 hours of missing data"),
#     tableOutput("longmissingTable")
#   )
# })
# 
# uiOutput("longmissingTab")
...
flux1$date <- as.Date.POSIXct(flux1$date_time, format="%Y-%m-%d")
  ...
  c_flux_goodvals <- subset(c_flux_sub, variable == input$variable1)
  
  # fluxPgood <- ggplot(c_flux_goodvals, aes(date_time, value)) +
  #   geom_line() +
  #   labs(y = input$variable1) +
  #   if(grepl("_IR", input$variable1)){
  #     lims(y=c(c_flux_goodvals$min, c_flux_goodvals$max)
  #   } else {
  ...
  if (grepl("_IR", input$variable1)) {
    fluxPgood <- fluxPgood + lims(y = c(min(na.omit(c_flux_goodvals$value)), max(na.omit(c_flux_goodvals$value))))
  } else {
    fluxPgood <- fluxPgood + lims(y = c(input$range_flux_min, input$range_flux_max))
  }
  
  # Prepare Biomet Plot
  c_biomet <- flux[date >= input$daterange[[1]] & date <= input$daterange[[2]], 
                   .SD, .SDcols = c("date", "date_time", "rain_Tot")]
  setnames(c_biomet, "rain_Tot", "selected")
  
  BiometP1 <- ggplot(c_biomet, aes(date_time, selected)) +
    geom_col() +
    labs(y = "total P.", title="biomet variable test") +
    theme_bw()
 ...
 # c_flux_goodvals <- subset(c_biomet_sub, !value >= max & !value <= min & variable == "rain_Tot")
  ...
  # pick up HERE
   simpVar_key <- data.frame("OldVariable"=c("AirTC_1p35", "AirTC_8p65", "AirTC_16_Avg",
                                             "AirTC_24_Avg", "CNR1TC_Avg", 
                                             "RH_1p35_Avg", "RH_8p65_Avg", "RH_16_Avg",
                                             "RH_24_Avg", "e_hmp_mean_Avg", "e_sat_Avg", "h2o_hmp_mean_Avg",
                                             ...
                                             "rslt_wnd_spd", "std_wnd_dir", "WS_ms_4p95_Avg", "WS_ms_21p65_Avg",
                                             "WS_ms_4p95_WVc(1)",
                                             "WS_ms_4p95_WVc(2)", "WS_ms_4p95_WVc(3)", 
                                             "WS_ms_21p65_WVc(1)", "WS_ms_21p65_WVc(2)",
                                             "WS_ms_21p65_WVc(3)", "Wind_Sentry_4p95_WVc(1)", "Wind_Sentry_4p95_WVc(2)",
                                             "Wind_Sentry_21p65_WVc(1)", "Wind_Sentry_21p65_WVc(2)", "NetTot_Avg", "par_faceup_Avg", 
                                             "par_facedown_Avg", "TargmV_Avg", "SBTempC_Avg", "TargTempC_Avg", 
                                             "Albedo_Avg"),
                             "CombinedPlotVar"=c(rep("AirTC", 5), rep("RH", 4), rep("rH_hmp", 3), rep("SW", 3), 
                                                 rep("LW", 3), rep("LW",2), rep("calcWS", 7),
                                                 rep("rawWS", 6), rep("rawWSentry",4), "NetTot_Avg", rep("PAR", 2),
                                                 rep("infrared", 3), "Albedo"))
   # albedo just added - check me
  ...
  micromet_sub <- subset(m_flux_sub2, SimpleVar == input$micrometvars)
    ...
    micromet1 <- ggplot(micromet_sub, aes(date_time, value, color=variable)) +
    geom_line() +
  ...
  p1b <- ggplotly(fluxPgood) %>% layout(showlegend = FALSE)
  p3b <- ggplotly(biometPgood) %>% layout(showlegend = FALSE)
  p4  <- ggplotly(micromet1) %>%
    layout(
      showlegend = TRUE,
      legend = list(
        orientation = "v",
  ...
  p5 <- ggplotly(DxPlot) %>% layout(showlegend = FALSE)
  p6 <- ggplotly(PowerDxPlot) %>% layout(showlegend = FALSE)
  # Combine with subplot and set layout properties
  #p5,
  ## this was working. 
   plotly::subplot(p1b, p3b, p5, p6, p4, nrows = 5, shareX = TRUE, titleY = TRUE) %>%
     layout(
       showlegend = TRUE,
        height = 1200,  
        margin = list(
...
renderPlotly({
  ...
  SonicP2 <- ggplot(c_sonic_sub2, aes(date_time, value)) +
    ...
    #labs(y = variable) +
    #lims(y=c(input$range_sonic2_min, input$range_sonic2_max)) + 
    theme_bw()
 ...
 # c_flux_goodvals <- subset(c_biomet_sub, !value >= max & !value <= min & variable == "rain_Tot")
  ...
  # 
  # # Combine with subplot and set layout properties
  # plotly::subplot(p2, p3, p4, nrows = 3, shareX = TRUE, titleX = TRUE, titleY = TRUE) %>%
  #   layout(
  #     #height =1500, 
...
output$longmissingTableco2h2o <- renderTable({
  req(input$irgavariable)

  tab <- NULL
  if (grepl("_IR", input$irgavariable)) {
    if (grepl("co2_um_m_Avg_IR", input$irgavariable)) tab <- CO2_long_na_periods
    if (grepl("h2o_mm_m_Avg_IR", input$irgavariable)) tab <- h2o_long_na_periods
  }
...
output$longmissingTableco2h2o <- renderUI({
  req(input$irgavariable)
  tagList(
    h4("Periods with more than 4 hours of missing data"),
    tableOutput("longmissingTable")
...
# print start/stop dat times for periods > 4 hours with missing or out of range data.
 ...
 #   irgatab <- NULL
 #   if (grepl("_IR", input$irgavariable)) {
 #     if (grepl("co2_um_m_Avg_IR", input$irgavariable)) irgatab <- CO2_long_na_periods
 #     if (grepl("h2o_mm_m_Avg_IR", input$irgavariable)) irgatab <- h2o_long_na_periods
 #   }
 # 
 #   if (!is.null(irgatab) && nrow(irgatab) > 0) {
 #     irgatab %>%
 #       mutate(
 #         start_time = format(as.POSIXct(start_time, origin = "1970-01-01", tz = "UTC"),
 #                             "%Y-%m-%d %H:%M:%S"),
 ...
 #   } else {
 #     # Return empty table with message
 #     data.frame(Message = "No periods out-of-range for > 4 consecutive hours (CO2 um/m or H2O mm/m).")
 #   }
 # })
 # 
 # output$longmissingTableCO2H2O <- renderUI({
 ...
 #   tagList(
 #     h4("Periods with more than 4 hours of missing data"),
 #     tableOutput("longmissingTableCO2H2O")
 #   )
 # })
 # 
 # uiOutput("longmissingTableCO2H2O")
...
# Wind direction from Sonic Anemometer
 ...
 # plot wind variables in windrose
  ...
  recentflux <-  flux %>%
   filter((date >= input$daterange2[[1]] & date <= input$daterange2[[2]])) 
   wind4p95 <- with(recentflux, windrose(WS_ms_4p95_Avg, WindDir_4p95_Avg,
                       speed_cuts = c(0.5, 1, 3, 5),
                       legend_title = "4p95 Wind Speed\n(m/s)",
                       legend.title.align = .5)) + 
     theme_minimal()
   
   wind21p65 <- with(recentflux, windrose(WS_ms_21p65_Avg, WindDir_21p65_Avg,
                       speed_cuts = c(0.5, 1, 3, 5),
                       legend_title = "21p65 Wind Speed\n(m/s)",
                       legend.title.align = .5)) + 
     theme_minimal()
   
   ggarrange(wind4p95, wind21p65)
  #wp1 <- ggplotly(wind4p95) 
  #wp2 <- ggplotly(wind21p65)
  # Combine with subplot and set layout properties
  # plotly::subplot(wp1, wp2, nrows = 2, shareX = TRUE, titleX = TRUE, titleY = TRUE) %>%
  #   layout(
  #      margin = list(
  #       l = 80,   # Left margin
  #       r = 200,  # Increased right margin to accommodate legends
  ...
  #     )
 # plot wind variables in windrose
   # plot.windrose(wind.data,wind.data$WS_ms_21p65_Avg,wind.data$WindDir_21p65_Avg) +
   # labs(title="CSAT3")+
   #   theme(legend.position="bottom")
 })
