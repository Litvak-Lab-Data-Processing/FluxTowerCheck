...
file_ids <- c("16uw93otKVyFnWjNIdToXfm-md-PAyHF1")  # file ID from google drive 
...
# soil pit type column
soil$pit_type <- ifelse(grepl("^J", soil$pit), "juniper", "open")

#fix soil pit
soil$pit <- str_replace(soil$pit, "03", "O3")

# add pit number 
soil$pit_number <- str_extract(soil$pit, "\\d+")
...
sapTrees <- sapTrees %>%
  separate(tree_probe, into = c("tree", "probe"), sep = "(?<=\\d)(?=\\D)")
...
radioButtons("variable1", "Flux variables:",
             c("CO2 Flux WPL (mg/m2/sec)" = "Fc_wpl",
               ...
               "H2O mean concentration (g/m3)" = "h2o_mean_Avg",
               "tau" = "tau",
               "U star" = "u_star"))
```
...
radioButtons("variable2", "Biomet variables:",
             c("Air temperature, hmp (C)" = "RTD_temp_Avg", 
               ...
               "Soil Heat Flux 1 (tree) (W/m2)" = "shf_j1_Avg",
               "Soil Heat Flux 2 (tree) (W/m2)" = "shf_j2_Avg",
               "Soil Heat Flux 3 (open) (W/m2)" = "shf_o1_Avg",
               "Soil Heat Flux 4 (open) (W/m2)" = "shf_o2_Avg",
              "Wind Speed (m/s)" = "wnd_spd",
              "Wind Direction (Compass)" = "wnd_dir_compass",
              "Wind Direction (CSAT)" = "wnd_dir_csat3",
              "Relative Humidity 3.7" = "RH_3p7_Avg",
              ...
              "AirTC_3p7_Avg" = "AirTC_3p7_Avg",
              "AirTC_10_Avg" = "AirTC_10_Avg",
              "WindDir_3p7_Avg" = "WindDir_3p7_Avg",
              "WS_ms_3p7_Avg" = "WS_ms_3p7_Avg",
              "WS_ms_3p7_WVc(1)" = "WS_ms_3p7_WVc(1)",
              "WS_ms_3p7_WVc(2)" = "WS_ms_3p7_WVc(2)",
              "WS_ms_3p7_WVc(3)" ="WS_ms_3p7_WVc(3)",
...
radioButtons("variable3", "Instrument variables:",
             c("Battery bank avg (V)" = "battbank_Avg",
               "Battery 1000 avg (V)" ="batt_1000x_Avg",
               ...
               "net tot avg" = "NetTot_Avg",
               "CO2 N tot" = "CO2_n_Tot",
               "H2O N tot" = "H2O_n_Tot",
               "watchdog 1000" = "watchdog_1000x_Tot",
               "low 12V 1000 Tot" = "low12V_1000x_Tot",
               ...
               "StarDotV_Avg" = "StarDotV_Avg",
               "Camera Under V tot" = "cameraUnderV_Tot",
               "Camera Over V Tot" = "cameraOverV_Tot",
...
# radioButtons("variable2_2", "Pit type:",
#              "juniper" = "juniper",
#              "open" = "open")
...
# radioButtons("variable2_3b", "Soil Depth:",
#              c("5cm"="5",
#                "10cm"= "10",
#                "30cm"="30"))
...
radioButtons("variable4_3", "Biomet variables:",
             c("Air temperature, hmp (C)" = "RTD_temp_Avg",
               ...
               "Atmospheric pressure (kPa)" = "press_Avg",
               "Precipitation (mm)" = "rain_Tot",
                "PAR out (W/m2)" = "par_facedown__Avg",
               "PAR in (W/m2)" = "par_faceup__Avg",
               "Short Wave IN (W/m2)" = "Rad_short_Up_Avg",
               "Short Wave OUT (W/m2)" = "Rad_short_Dn_Avg",
               "Long Wave IN (W/m2)" = "Rad_long_Up_Avg",
               "Long Wave OUT (W/m2)" = "Rad_long_Dn_Avg",
...
dateRangeInput(
  "daterange5",
  "Date range:",
  start = max(flux$date) - 14,  # Start 2 weeks before the maximum date
  end = max(flux$date),         # End at the maximum date
  min = min(flux$date),         # The earliest possible date
  max = max(flux$date)          # The latest possible date
)
...
# # Populate variable choices dynamically
# observe({
#   updateSelectInput(
#     session,
#     "selected_variable",
#     choices = unique(results()$variable)
#   )
# })
...
#   results() %>%
#     summarize(
#       total_variables = n_distinct(variable),
#       total_out_of_range_hours = sum(out_of_range_hours, na.rm = TRUE)
#     )
# })

# Interactive plot for out-of-range data
renderPlotly({
  fluxfilt <-  flux %>%
    filter(date >= input$daterange5[[1]] & date <= input$daterange5[[2]])

  fluxdt <- as.data.table(fluxfilt)
  ...
  results <- fluxdt %>%
    ...
    left_join(lims, by = "variable") %>%
    mutate(out_of_range = is.na(value) | value < min | value > max) %>%
    group_by(variable) %>%
    summarize(
      out_of_range_hours = sum(out_of_range) * 0.5,
      timestamps = (list(date_time[out_of_range]))[1]
    ) %>%
    filter(out_of_range_hours > 4)
 ...
 fluxL <-  fluxdt %>%
  ...
  plot_data <- fluxL %>%
    filter(variable %in% results$variable)
  
  plot_data$min <- lims$min[match(plot_data$variable, lims$variable)]
  plot_data$max <- lims$max[match(plot_data$variable, lims$variable)]

  
  plot_data$pt_type <- ifelse(plot_data$value >= plot_data$max | plot_data$value <= plot_data$min, "out-of-range", "ok")
  
  gg <- ggplot(plot_data, aes(date_time, value)) +
    geom_line() +
    geom_point(aes(color=pt_type)) +
    facet_wrap(.~variable, scales = "free_y") +
    labs(title = "Out-of-Range Data", x = "Timestamp", y = "Value") +
    theme_minimal()
...
#   flux %>% filter(variable == input$selected_variable)
# })
# 
# output$variable_plot <- renderPlotly({
#   req(data_filtered())
#   gg <- ggplot(data_filtered(), aes(date_time, value)) +
#     geom_line() +
#     labs(title = paste("Time Series for", input$selected_variable), x = "Time", y = "Value") +
#     theme_minimal()
# 
#   ggplotly(gg)
# })
