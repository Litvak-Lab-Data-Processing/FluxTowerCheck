...
## list file ids and names
file_ids <- c("16atuTrnsZOQmXkBAtldTuzAheuAcP39-", "16hHe-HB0DqY8ptFdk2ymAmHsrcgvNOEJ")  # file ID from google drive 
...
lims <- read.csv(paste0(basedir, "MpjVarLims.csv"))
...
colnames(sap_trees) <- str_replace(colnames(sap_trees), "(sap_P\\d+)([A-Za-z])", "\\1_\\2")
...
sapTrees$species <- ifelse(grepl("P", sapTrees$tree), "pinon", "juniper")
...
flux1$date <- as.Date.POSIXct(flux1$date_time, format="%Y-%m-%d")
  ...
  # key for plotting multiple biomet in a single category
   simpVar_key <- data.frame("OldVariable"=c("AirTC_10_Avg", "CNR1TC_Avg", 
                                             "RH_10_Avg", 
...
dateRangeInput("daterange3", "Date range:",
                 start = max(soil1$date) - weeks(2),
                 end   = max(soil1$date),
                 min = max(soil1$date)- weeks(2),
                 max = max(soil1$date))
...
#                    .SD, .SDcols = c("date", "date_time", input$variable3_3)]
# 
#   BiometP1 <- ggplot(c_biomet, aes(date_time, input$variable3_3)) +
#     geom_line() +
#     labs(y = input$variable3_3) +
#     theme_bw()
#     p4 <- ggplotly(BiometP1) %>%
...
renderPlotly({
   ...
   sap_species <- ggplot(sapdata, aes(date_time, value, colour = tree)) +
     geom_line() +
     facet_grid(species ~ ., scales = "free_y") +
     labs(y = "sap flow, all trees") +
     theme_bw() +
     theme(legend.position = "bottom")
   
   #one sap tree in plot
   sap_data <- sapTrees %>%
     filter((date >= input$daterange4[[1]] & date <= input$daterange4[[2]]) & 
              tree==input$variabletreenum)
   
   sap_tree <- ggplot(sap_data, aes(date_time, value, colour=probe))+
     geom_line()+
     labs(y="sap flow for selected tree")+
     theme_bw()+
     theme(legend.position="bottom")
  ...
  # Convert to plotly objects
  p1 <- ggplotly(sap_species) 
  p2 <- ggplotly(sap_tree) 
  p3 <- ggplotly(soilP) 
  p4 <- ggplotly(BiometP1) 
