...
lims <- read.csv(paste0(basedir, "WjsVarLims.csv"))
...
# rename to split into tree and probe
...
flux$Fc_wpl_IR <- ifelse(flux$Fc_wpl < -50 | flux$Fc_wpl >50, NaN, flux$Fc_wpl)
flux$Hc_IR <- ifelse(flux$Hc < -500 | flux$Hc > 800, NaN, flux$Hc)
flux$LE_wpl_IR <- ifelse(flux$LE_wpl < -150 | flux$LE_wpl > 300, NaN, flux$LE_wpl)
flux$co2_um_m_Avg_IR <- ifelse(flux$co2_um_m_Avg < 400 | flux$co2_um_m_Avg > 460, NaN, flux$co2_um_m_Avg)
flux$h2o_mm_m_Avg_IR <- ifelse(flux$h2o_mm_m_Avg < 0 | flux$h2o_mm_m_Avg > 100, NaN, flux$h2o_mm_m_Avg)
...
c_flux_with_na_runs <- flux1 %>%
  arrange(date_time) %>%
  mutate(is_na = is.na(Fc_wpl_IR) | Fc_wpl_IR == "NaN")
...
radioButtons("power_dx", "Diagnostic variables:",
             c("panelT_1000x_Avg" = "panelT_1000x_Avg",
               ...
               "cameraOverV_Tot" = "cameraOverV_Tot",
               "cameraUnderV_Tot" = "cameraUnderV_Tot"))
...
flux1$date <- as.Date.POSIXct(flux1$date_time, format="%Y-%m-%d")
  ...
  # key for plotting multiple biomet in a single category
   simpVar_key <- data.frame("OldVariable"=c("AirTC_3p7_Avg", "AirTC_10_Avg", "CNR1TC_Avg", 
                                             "RH_3p7_Avg", "RH_10_Avg", 
                                             ...
                                             "rslt_wnd_spd", "std_wnd_dir", 
                                             "WS_ms_3p7_Avg", 
                                             "WS_ms_3p7_WVc(1)", "WS_ms_3p7_WVc(2)", "WS_ms_3p7_WVc(3)", 
                                             "NetTot_Avg", "par_faceup_Avg", 
                                             "par_facedown_Avg", "TargmV_Avg", "SBTempC_Avg", "TargTempC_Avg",
                                             "IRT_J1_Avg" = "IRT_J1_Avg",
                                             "IRT_J2_Avg" = "IRT_J2_Avg",
                                             "IRT_J3_Avg" = "IRT_J3_Avg",
                                             "IRT_O1_Avg" = "IRT_O1_Avg"),
                             
                             "CombinedPlotVar"=c(rep("AirTC", 3), rep("RH", 2), rep("rH_hmp", 3), rep("SW", 3), 
                                                 rep("LW", 3), rep("LW",2), rep("calcWS", 6),
                                                 rep("rawWS", 3),  "NetTot_Avg", rep("PAR", 2),
                                                 rep("infrared", 3), rep("IRT", 4)))
...
# Wind direction from Sonic Anemometer
 renderPlot({
  recentflux <-  flux %>%
   ...
   wind3p7 <- with(recentflux, windrose(WS_ms_3p7_Avg, WindDir_3p7_Avg,
